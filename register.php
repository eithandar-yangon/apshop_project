...
if($_POST){
	if(empty($_POST['name']) || empty($_POST['email']) || empty($_POST['phone']) || empty($_POST['address']) || empty($_POST['password']) || strlen($_POST['password']) < 4 ){
		...
		echo "<script>alert('Email already existed')</script>";
			...
			$result = $stmt->execute(
				array(':name'=>$name,':email'=>$email,':phone'=>$phone,':password'=>$password,':address'=>$address)
			);
			...
			<div class="row">
				...
				<div class="col-lg-6">
					<div class="login_form_inner">
						<h3>Register | Enjoy Shopping</h3>
						<form class="row login_form" action="register.php" method="post" id="contactForm" novalidate="novalidate">
							<input type="hidden" name="_token" value="<?php echo $_SESSION['_token']; ?>">
							<div class="col-md-12 form-group">
								<input type="text" class="form-control" id="name" name="name" placeholder="Username" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Username'" style="<?php echo empty($nameError) ? '': 'border-bottom:1px solid red';  ?>">
								<p style="color:red;float: left;"><?php echo empty($nameError) ? '': '*'. $nameError  ?></p>
							</div>
							<div class="col-md-12 form-group">
								<input type="text" class="form-control" id="name" name="email" placeholder="Email" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Email'" style="<?php echo empty($emailError) ? '': 'border-bottom:1px solid red';  ?>">
							...
							<div class="col-md-12 form-group">
								<input type="number" class="form-control" id="name" name="phone" placeholder="Phone" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Phone'" style="<?php echo empty($phoneError) ? '': 'border-bottom:1px solid red';  ?>">
								<p style="color:red;float: left;"><?php echo empty($phoneError) ? '': '*'. $phoneError  ?></p>
							</div>
							<div class="col-md-12 form-group">
								<input type="text" class="form-control" id="name" name="address" placeholder="Address" onfocus="this.placeholder = ''" onblur="this.placeholder = 'Address'" style="<?php echo empty($addressError) ? '': 'border-bottom:1px solid red';  ?>">
								<p style="color:red;float: left;"><?php echo empty($addressError) ? '': '*'. $addressError  ?></p>
							</div>
